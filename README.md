...
- `resource(string $path, string $controller, array $except = [], array $middlewares = [], array $routeMiddlewares = [], ?string $namePrefix = null, bool $appendRestoreMethod = false, array $validators = [], string $routeParameter = 'id')`
...
$router->urlFor('user.show', ['user' => $userEntity]); // "/users/42"
```
...
По умолчанию используется `EntityManagerInterface::find()`. Чтобы разрешать soft‑deleted записи, можно отметить параметр атрибутом `#[WithDeleted]`:

```php
use PhpSoftBox\Router\Attributes\WithDeleted;

public function show(#[WithDeleted] User $user): ResponseInterface
{
}
```
...
Чтобы автоматически проверять связь между вложенными сущностями, используйте `scopeBindings()`/`scoped()`:
