...
final class ContainerHandlerResolver implements HandlerResolverInterface
{
    ...
    private function resolveEntity(
        ...
        if ($this->parameterWithDeleted($parameter)) {
            $repo = $entityManager->repository($typeName);
            if (is_object($repo) && method_exists($repo, 'findWithDeleted')) {
                return $repo->findWithDeleted($value);
            }
        }

        return $entityManager->find($typeName, $value);
    }

    private function parameterWithDeleted(ReflectionParameter $parameter): bool
    {
    ...
    private function resolveEntityManager(): ?object
    {
