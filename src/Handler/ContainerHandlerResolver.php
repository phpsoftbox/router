...
final class ContainerHandlerResolver implements HandlerResolverInterface
{
    ...
    private function wrapCallable(callable $callable): callable
    {
        return function (ServerRequestInterface $request) use ($callable) {
            if (method_exists($this->container, 'call')) {
                ...
                [$request, $params, $routeParams] = $this->resolveRouteEntities(
                    $ref,
                    $request,
                    $params,
                    $routeParams,
                ...
                foreach ($ref->getParameters() as $parameter) {
                    ...
                    if (
                        ...
                        $schema = $this->makeRequestSchema($typeName, $appRequest);
    ...
    private function makeRequestSchema(string $typeName, Request $appRequest): RequestSchema
    {
        if (method_exists($this->container, 'make')) {
            $schema = $this->container->make($typeName, $this->resolveRequestSchemaMakeParameters($typeName, $appRequest));
            if (!$schema instanceof RequestSchema) {
                throw new RuntimeException('Container make must return RequestSchema instance for: ' . $typeName);
            }

            return $schema;
        }

        return new $typeName($appRequest);
    }

    /**
     * @param array<string, mixed> $params
     * @param array<string, mixed> $routeParams
     * @return array{0: ServerRequestInterface, 1: array<string, mixed>, 2: array<string, mixed>}
     */
    private function resolveRouteEntities(
        ...
        foreach ($ref->getParameters() as $parameter) {
            ...
            $routeParamExists = array_key_exists($name, $routeParams);
            ...
            $params[$name]      = $entity;
            $routeParams[$name] = $entity;
            $request            = $request
                ->withAttribute($name, $entity)
                ->withAttribute('_route_params', $routeParams);
        }

        return [$request, $params, $routeParams];
    }

    /**
     * @return array<string, Request>
     */
    private function resolveRequestSchemaMakeParameters(string $typeName, Request $appRequest): array
    {
        ...
        foreach ($constructor->getParameters() as $parameter) {
            ...
            $parameters[$parameter->getName()] = $appRequest;
        }
    ...
    private function resolveEntity(
        ...
        // Route params are authoritative even when middleware has stored an entity
        // under the same request attribute name (for example, the authenticated user).
        if ($routeParamExists) {
            $value = $routeValue;
        ...
        if (is_object($value)) {
            return is_a($value, $typeName) ? $value : null;
        }
        ...
        if ($metadata['column'] !== null) {
            $entity = $this->findEntityByColumn(
                $entityManager,
                $typeName,
                $metadata['column'],
                $value,
                $metadata['withDeleted'],
            );
            if ($entity !== null) {
    ...
    /**
     * @return array{withDeleted: bool, with: list<string>, column: ?string}
     */
    ...
    private function findEntityByColumn(
        ...
        if (!method_exists($entityManager, 'queryFor') || !method_exists($entityManager, 'connection')) {
            throw new RuntimeException(
                'Entity binding by column requires EntityManagerInterface::queryFor() and connection().',
            );
        }

        $connection = $entityManager->connection();
        if (!is_object($connection) || !method_exists($connection, 'quoteIdentifier')) {
            throw new RuntimeException('Entity binding by column requires a connection with quoteIdentifier().');
        }

        $query = $entityManager->queryFor($entityClass, $withDeleted);
        if (!is_object($query) || !method_exists($query, 'where') || !method_exists($query, 'fetchEntity')) {
            throw new RuntimeException(
                'Entity binding by column requires an ORM query with where() and fetchEntity().',
            );
        }

        $quotedColumn = $connection->quoteIdentifier($column);
        $query->where([$quotedColumn => $value]);

        $entity = $query->fetchEntity();

        return is_object($entity) && is_a($entity, $entityClass) ? $entity : null;
    }
    ...
    private function isScopedByOrmMetadata(object $parent, object $child): bool
    {
        ...
        if ($entityManager === null || !method_exists($entityManager, 'connection')) {
            return false;
        }

        $metaProvider = $this->resolveMetadataProvider($entityManager);
    ...
    private function resolveMetadataProvider(object $entityManager): ?object
    {
        if (!method_exists($entityManager, 'metadataProvider')) {
            return null;
        }

        try {
            $metadataProvider = $entityManager->metadataProvider();
        } catch (Throwable) {
            return null;
        }

        if (!is_object($metadataProvider) || !method_exists($metadataProvider, 'for')) {
            return null;
        }
