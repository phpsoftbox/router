...
final readonly class UrlGenerator implements UrlGeneratorInterface
{
    ...
    public function generate(string $name, array $params = [], bool $shouldAbsolute = false, ?string $host = null): string
    {
        ...
        return $this->buildAbsoluteUrl($route, $path, $host);
    }
    ...
    private function buildAbsoluteUrl(Route $route, string $path, ?string $host = null): string
    {
        ...
        if ($host !== '') {
            return $this->buildConfiguredHostUrl($host, $path);
        }

        $contextHost = trim($this->context->getHost());
        if ($route->hosts !== []) {
            if ($contextHost !== '' && RouteHost::contains($route->hosts, $contextHost)) {
                return $this->buildContextHostUrl($contextHost, $path);
            }

            $routeHost = RouteHost::canonical($route->hosts);

            return $routeHost !== null ? $this->buildConfiguredHostUrl($routeHost, $path) : $path;
        }
    ...
    private function buildContextHostUrl(string $contextHost, string $path): string
    {
    ...
    private function buildConfiguredHostUrl(string $host, string $path): string
    {
